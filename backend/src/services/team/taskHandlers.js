...
// переводим в нижний регистр. Кириллица проходит как есть (в \w + UNICODE).
// Для текстов точек экскурсии (где русские буквы в названии) это критично —
// иначе всё превратится в "untitled".
function slugify(text, maxLen = 40) {
  let s = String(text ?? "")
    .normalize("NFKD")
    // Удаляем всё, что не является буквой/цифрой/пробелом/дефисом/подчёркиванием.
    // \p{L}\p{N} — Unicode-классы (поддерживаются в Node 12+).
    .replace(/[^\p{L}\p{N}\s_-]/gu, "")
    // Пробелы/нижние подчёркивания/дефисы → один дефис.
    .replace(/[\s_-]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .toLowerCase();
  if (!s) s = "task";
