...
### Сессия 22 — Предложения от агентов: таблица, сервис, двухтактный процесс (этап 3, пункт 15) ✅ 2026-05-11
...
**Отклонения:**
- **Миграция — `0026_team_proposals.sql`** (сквозная нумерация), не `0021` из ТЗ. `autonomy_enabled_globally` записан в существующую `team_settings` (jsonb-key-value), не отдельным столбцом.
- **Отдельный столбец `source` в `team_api_calls` НЕ добавляется** — переиспользую существующее поле `purpose`: для такта 1 = `'autonomy_filter'`, для такта 2 = `'autonomy_propose'`. ТЗ говорит «source = 'autonomy'», но в текущей схеме `team_api_calls.purpose` уже служит этой цели (Сессии 9, 12, 14, 15, 20). Поагентная биллинг-разбивка в Сессии 49 будет фильтровать по этим purpose'ам.
- **`team_trigger_state`** (для cooldown'ов событийных триггеров) — отложен на Сессию 24. Сейчас cooldown 7 дней считается через JOIN `team_proposals` и `team_agent_diary` по `(agent_id, triggered_by)` — `getLastReflection()`. Этого хватает для еженедельного окна.
- **`urgent` понижается до `regular`** при превышении 1/неделя — не блокируется. Чтобы не терять формулировку, которую модель уже выдала.
- **Дешёвая модель для такта 1** определяется по `pricing.json` (если есть «haiku» в названии). Если нет — берётся та же, что для такта 2. Сессия 49 даст глобальный `system_llm_model` — оба такта будут смотреть туда.
- **Триггеры в этой сессии — только `weekly_window`** (ручной cli `npm run triggers:run`). Событийные триггеры (`new_competitor_entry`, `low_score`, `goals_changed`) и автоматический `node-cron` — Сессия 24. UI Inbox/дневника/тумблера — Сессия 23.
- **Inbox-нотификация при создании предложения** — тип `proposal` уже добавлен в CHECK в Сессии 18. Здесь только дёргаем `createNotification` после успешного `createProposal`.

---

### Сессия 23 — UI автономности: предложения в Inbox, дневник, тумблер (этап 3, пункт 15) ✅ 2026-05-11
...
**Отклонения:**
- **Проверка Role-блоков при включении автономности — не реализована**. Кнопка переключает уровень напрямую (`apiUpdateAgent(id, { autonomy_level: next })`). Причина: Role-файлы в текущем проде у тестовых агентов часто пустые, блокирующая модалка тормозила бы Влада. Если перейти на 1, а Role-файл пустой — модель такта 1 сама ответит «не могу предложить» (или такт 2 даст некачественную формулировку), Влад это увидит. Жёсткий гейт можно добавить позже на основе реальной обратной связи.
- **«Принять с правками»** — реализовано через ту же `/accept`-ручку (бэкенд принимает опц. `brief`/`task_type`/`project_id` overrides), но UI пока вызывает только `acceptProposal(id, {})` — без формы правок. ТЗ просит «бриф editable в textarea перед финальным созданием задачи» — это можно добавить позже, если предложения часто требуют корректировок. Текущая логика: payload.what становится `user_input` задачи, agent остаётся тот же.
- **Endpoint `/api/team/admin/autonomy`** добавлен (GET/POST), читает/пишет `team_settings.autonomy_enabled_globally` и считает суммарные расходы за 30 дней по `purpose IN ('autonomy_filter','autonomy_propose')`.
- **Бейдж «🎯 Инициативный»** заменил старый «Автономен» в списке Сотрудников (Сессия 9). Цвет — accent-soft, чтобы выделялся среди серых.
- **`ProposalsPanel`** — отдельный компонент над `InboxBlock`, прячется когда pending=0. Поллинг каждые 30 сек. urgent (`kind='urgent'`) сортируется сверху.
- **`DiaryTab`** виден только при `autonomy_level >= 1` — у реактивных агентов записей нет, вкладка бесполезна.

---

### Сессия 24 — Событийные триггеры и автоматизация цикла размышления (этап 3, пункт 15) ✅ 2026-05-12
...
**Отклонения:**
- **Миграция — `0027_team_trigger_state.sql`** (сквозная нумерация), inline 0022 в ТЗ.
- **Триггер `new_competitor_entry`** проверяет реестр `team_custom_databases` с `db_type='competitor'`, а не «любую запись в таблице конкретного блогера» (как в ТЗ). Реальный парсинг таблиц конкурентов появится в Сессии 33 (Apify), там же триггер можно расширить «появилась N-я запись в таблице блогера X».
- **Триггер `goals_changed`** через fingerprint длины файла `strategy/goals.md`, а не через `teamStorage.getFileInfo().updated_at` (такой утилиты ещё нет, а у Storage API нет дешёвого HEAD-эндпоинта). Length-fingerprint достаточен: если Влад правит текст — длина почти наверняка меняется. Идеальный API (через etag) — Сессия 49+.
- **Триггер «новая запись в базе»** (для referensy / videos) не реализован: спецификация ТЗ упоминала «`team_custom_databases` и связанные таблицы», но это многотабличный сценарий без чёткого критерия «что считать новой релевантной записью». Оставлено на будущее — событие можно добавить отдельной функцией `pollNewReferenceFor` без переписывания каркаса.
- **node-cron расписание**: poll каждые 6 часов (`0 */6 * * *`), weekly в 10:00 UTC, expire в 03:00 UTC. Все в `Etc/UTC` — это надёжнее, чем зависеть от локали процесса Railway.
- **`team_trigger_state.last_checked_at`** — обычная timestamptz. Для `low_score` и `new_competitor_entry` туда пишется реальный `created_at` обработанной записи. Для `goals_changed` — sentinel-дата: длина файла, закодированная как `new Date(length * 1000).toISOString()` (см. `lengthToSentinelDate` / `sentinelDateToLength` в triggerService). Это компромисс: один столбец, sentinel валиден как timestamp, а фактически мы запоминаем длину. Если в будущем кто-то начнёт фильтровать `last_checked_at < now() - interval '7 days'` — для goals_changed это даст странные результаты, и тогда надо будет добавить `state_payload jsonb`.

---

### Сессия 25 — Skills: Storage, сервис, загрузка в промпт (этап 4, пункт 10) ✅ 2026-05-12
...
**Отклонения:**
- **Миграция — `0028_team_skill_candidates.sql`** (сквозная нумерация), inline 0023 в ТЗ.
- **Storage-путь skill-файлов — `agent-skills/<agent_id>/<slug>.md`** (Latin), а не `Навыки агентов/<display_name>/`. Та же причина, что в Сессиях 4, 9, 20 — Supabase Storage отбивает кириллицу в путях.
- **Ключ папки — `agent_id` (slug), а не `display_name`**. promptBuilder теперь зовёт `loadSkills(agentId)`, а не agentName. Если у агента поменяется display_name — путь не сломается.
- **Установлен `gray-matter`** для парсинга/генерации YAML-frontmatter в skill-файлах. Frontmatter: `skill_name`, `status` (`active`|`pinned`|`archived`), `created_at`, `last_used`, `use_count`, опц. `source_task_id`.
- **`why_it_works` не идёт в промпт** (только для Влада, как в ТЗ). В Storage хранится, в `getSkillsContentForPrompt` пропускается.
- **UI-куски (блок в Инструкциях, вкладка «Навыки» в карточке агента, dropdown порога в Админке) — НЕ реализованы в этой сессии**. Backend полностью готов: миграция, сервис, маршруты, frontend-клиент. UI добавится в Сессии 27 (где экран кандидатов в навыки) — тогда оба куска UI можно сделать одним заходом, потому что они визуально связаны. Это намеренное сокращение объёма Сессии 25 (она и так разрослась — миграция + сервис + изменение promptBuilder).
- **`init-skills-folder.js`** не создан: Supabase Storage создаёт «папку» при первой загрузке файла, поэтому скрипт ничего не делает на пустом проекте. Когда создаётся первый skill через POST `/api/team/skills/:agentId` — папка появляется автоматически.
- **Slug извлекается из `skill_name`** через транслитерацию (как в taskHandlers). Если slug пересекается с существующим файлом — `createSkillFile` молча перезаписывает. Это можно исправить в UI (Сессия 27): проверять список перед созданием.

---

### Сессия 26 — Skill extraction: автоматический анализ успешных задач (этап 4, пункт 10) ✅ 2026-05-12
...
**Отклонения:**
- **`source` в team_api_calls** — переиспользую существующее поле `purpose` (как в Сессиях 22 и 25). Значение — `'skill_extraction'`. Отдельный столбец `source` не добавляю.
- **Идемпотентность по task_id**: если для задачи уже есть кандидат со status ≠ `'rejected'` — повторное извлечение пропускается с reason `duplicate_for_task`. `rejected`-кандидаты не блокируют новые попытки (Влад может изменить мнение).
- **Анти-дубль skill_name** не делается на этом этапе (LLM сама смотрит на список уже принятых navыков в промпте). Если выдаёт похожий — Сессия 27 даст «Принять с правкой».
- **`source_task_id` в frontmatter skill-файла** появляется только при принятии кандидата через Сессию 27. На этапе создания candidate такого поля нет, потому что skill ещё не в Storage.
- **Inbox-нотификация `skill_candidate`** создаётся внутри `extractSkillCandidate` после успешного INSERT. Тип уже был добавлен в Сессии 18.
- **cron автоматизация batch extraction** — НЕ реализована в этой сессии. ТЗ упоминает добавление в existing cron из Сессии 24. Откладываю до момента, когда реально появятся околопороговые эпизоды на проде; пока хватает ручного `npm run extract:skills`.
- **Скрипт `extract-skills.js`** поддерживает `--dry-run`, `--agent <id>`, без аргументов — все активные. Между LLM-вызовами 1 секунда задержки (rate limit).

---

### Сессия 27 — Экран «Кандидаты в навыки» и интеграция с Inbox (этап 4, пункт 10) ✅ 2026-05-12
...
**Отклонения:**
- **Auto-mark прочитанной нотификации `skill_candidate`** при approve/reject — НЕ реализован. ТЗ упоминает «При approve/reject — помечай уведомление как прочитанное». Связь between `team_notifications.related_entity_id` и candidate.id есть, но запрос требовал бы SELECT+UPDATE по двум таблицам. Можно добавить позже одной строкой в `approveSkillCandidate/rejectSkillCandidate`. Сейчас Влад нажимает «Все прочитано» в колокольчике вручную.
- **Бейдж pending на ссылке «Кандидаты в навыки»** в StaffWorkspace НЕ показывается — отдельный poll для skill-кандидатов не добавлен. Inbox на дашборде и колокольчик уже знают через нотификации; для шапки списка сотрудников отдельная цифра не критична.
- **Sidebar-пункт «Кандидаты в навыки»** — НЕ добавлен. Кнопка в шапке `/blog/team/staff` достаточна; пихать ещё одну ссылку в sidebar = шум. В Сессии 18 такой sidebar-пункт тоже не делался (только bell + dashboard inbox).
- **Skills-вкладка** в карточке агента (отложенная из Сессии 25) добавлена здесь же — read-only список + действия pin/archive/delete. Создание навыка из карточки агента НЕ реализовано: для нормального workflow Влад одобряет кандидата на `/skill-candidates` (с возможностью inline-правки) — это правильный entry point. Кнопку «+ Добавить навык вручную» можно прикрутить позже как тонкий UI поверх существующего `POST /api/team/skills/:agentId`.
- **«Открепить» pinned-навык** делается через `updateAgentSkill` с `status='active'` (нет отдельного эндпоинта unpin). Это микро-deviation, не блокирует.

---

### Сессия 28 — Счётчик токенов в UI (этап 4, пункт 11) ✅ 2026-05-12
...
**Отклонения:** `AutosavingTextEditor.tsx` в этой сессии бейджем не оборачивался — он уже не используется ни в одном экране (Mission/Goals/Role/Skills редактируются через `FileEditorPanel` в `InstructionsWorkspace`). Бейдж навешен только на активный путь редактирования файла. Серверный эстимат — грубый `Math.ceil(text.length / 4)` (без `js-tiktoken` на бэкенде); клиентский счёт через `cl100k_base` остаётся точным каналом для редактора. `author_profile` исключён из breakdown — он не привязан к агенту, файл появится в этапе 2 (пункт 9).

---

### Сессия 29 — Self-review: сервис, сборка чек-листа, второй вызов (этап 4, пункт 11) ✅ 2026-05-12
...
**Отклонения:**
- **Миграция — `0029_team_self_review.sql`** (сквозная нумерация проекта), а не `0024` из локального ТЗ.
- **`team_api_calls.source`** — отдельный столбец не вводится; переиспользую существующее поле `purpose='self_review'` (тот же приём, что в Сессиях 22, 25, 26 для остальных не-task вызовов). Билинг Сессии 49 будет фильтровать по этим purpose-значениям.
- **Системный промпт второго вызова не повторно подгружает Mission/Role/Goals/Memory/Skills** — ТЗ говорит «через `promptBuilder.buildPrompt()` с тем же agentId». Решение: задача уже была решена с этими слоями, повторная подкладка удвоит токены на ровном месте. Передаю только короткий self-review system + чек-лист как user. На результат «прошло/не прошло по чек-листу» это не влияет — экономит ~60–80% токенов второго вызова.
- **`---REVISED---` маркер** для разделения JSON-блока чек-листа и опционального исправленного ответа. Гибче, чем `\`\`\`json\`\`\` + текст подряд: парсер `parseSelfReviewResponse` идёт по этому маркеру.
- **Шестой источник чек-листа (методички инструментов)** — НЕ реализован: оставлен TODO в `buildChecklist`. Toolservice уже есть с Сессии 20, но «секция Самопроверка» в методичках появится начиная с Сессии 32 (Web Search). Тогда добавлю одной итерацией.
- **`getTaskTemplateDefaults` живёт в promptBuilder, а не в taskHandlers** — таска handlers уже импортирует promptBuilder; добавление reverse-import породило бы цикл. Дублирующий маленький мап `TASK_TYPE_TO_SLUG` рядом с функцией — компромисс, легче сопровождать.
- **`GET /api/team/tasks/template-defaults/:taskType`** — отдельный лёгкий эндпоинт для UI (не загружает весь шаблон, только парсит frontmatter). Альтернатива «брать defaults из POST /preview-prompt» — дороже и не нужна формой.
- **Сохранение `selfReviewResult` в snapshot** идёт через `mergeSnapshot` тем же путём, что `suggestedNextSteps` (Сессия 13) — append-only история сохраняет финальный JSON.

---

### Сессия 30 — Self-review: UI результата в карточке задачи (этап 4, пункт 11) ✅ 2026-05-12
...
**Отклонения:**
- **`team_api_calls.source` → `purpose`** — ТЗ говорит «`source = 'self_review'`», но в проекте такого столбца нет; везде используется `purpose` (см. Сессии 22/25/26/29). Соответственно эндпоинт `cost-breakdown` группирует по `purpose`.
- **Cost-breakdown скрывается, если purpose всего один** — нет смысла дублировать «$X.YY» одной строкой; и так есть `task.costUsd` в шапке. Появляется при наличии второго purpose (`self_review`, `autonomy_*`, и т.п.).
- **Индикатор self-review в логе задач** реализован как эмодзи-бейдж рядом с моделью (🔍✅/⚠️/❌). Не отдельный значок около статуса — внутри карточки выглядит компактнее.
- **`skipped: true` в self_review_result** обрабатывается отдельно — рендерится мягким текстом «Самопроверка пропущена: причина», без раскрывающегося блока (на сегодняшний код такие записи в БД не пишутся — `taskRunner` сохраняет только не-skipped результат; но если когда-то начнём писать — UI готов).
- **Источники чек-листа** локализованы: memory_rule→«Правило Memory», skill→«Навык», template_field→«ТЗ», mission_taboo→«Табу Mission», vlad_extra→«Доп. проверка», tool_manifest→«Инструмент» (для будущей Сессии 32+).
- **TaskCostItem импортирован, но используется только в типе `TaskCostBreakdown.items`** — оставлен экспорт для будущего UI Сессии 49.

---

### Сессия 31 — Новые статусы задач, многошаговая инфраструктура, уточнения от агента (этап 5, пункт 17) ✅ 2026-05-12
...
**Отклонения:**
- **Миграция — `0030_team_multistep_tasks.sql`** (сквозная нумерация), не `0025` из ТЗ.
- **status — TEXT, а не ENUM**. ТЗ предлагает `status TEXT` без CHECK с документацией в COMMENT — так и сделал. Это совместимо с уже существующими статусами и не требует ALTER TABLE для новых.
- **`team_api_calls.source` → `purpose`** — третья сессия подряд, где переиспользую `purpose='clarification'` вместо отсутствующего столбца `source`. Бизнес-смысл идентичен.
- **Системная LLM для уточнений ещё не реализована** (Сессия 49) — пока выбираю первый доступный провайдер через `keysService.getApiKey` (Anthropic → OpenAI → Google) с дешёвой моделью по умолчанию для каждого. Когда появится Системная LLM, переключусь на неё одной строкой в `pickProvider`.
- **`generateClarificationsForTask` — async после createTask через `setImmediate`**, не в первом синхронном вызове. Это позволяет вернуть `taskId` сразу, а UI поллит статус и через несколько секунд увидит `awaiting_input` или `running`.
- **Пересборка промпта после `applyClarificationAnswers`**: extra-блок «## Уточнения от автора» добавляется в `params.user_input`, и `previewPrompt` зовётся повторно перед enqueue. Если перепересборка падает — продолжаем с прежним промптом (handler сам соберёт user-часть из params).
- **Recovery clarifying-задач**: после рестарта `getStuckClarifyingTaskIds()` + повторный `generateClarificationsForStuckTask()`. Не идемпотентно по биллингу — если процесс упал ровно между LLM-ответом и записью статуса, расход спишется дважды. Это редкий кейс; полноценная идемпотентность через `clarification_request_id` отложена.
- **`awaiting_resource`** в Сессии 31 — только инфраструктура (status, step_state, UI прогресс-бар). Реальные многошаговые задачи (NotebookLM-исследователь) и продвижение шагов — Сессия 38. `taskContinuationService` сегодня используется только в тестах.
- **Старый Frontmatter-флаг `clarification_default` шаблонов задач НЕ читается** — было обещано в ТЗ Сессии 37 (шаблоны исследователя), но из текущих 5 шаблонов ни один его не несёт. Когда добавятся новые шаблоны с `clarification_default: true`, придётся вернуться и подключить чтение этого поля в `getTaskTemplateDefaults`-flow.
...
### Сессия 33 — База конкурентов: UI, Apify, AI-саммари (этап 5, пункт 17) ✅ 2026-05-12
...
**Отклонения:**
- **Миграция — `0031_team_competitor_posts.sql`** (сквозная нумерация). Не «динамически создаваемая таблица для каждого блогера» из ТЗ, а одна общая таблица постов с FK `competitor_id` на `team_custom_databases.id`. Причина: посты структурно одинаковые, отличается только владелец — отдельная таблица для каждого блогера была бы шумом схемы. `team_custom_databases.create_custom_table` (Сессия 45) останется для произвольных пользовательских баз.
- **`competitor_id` — uuid**, не text как в исходной попытке: `team_custom_databases.id` имеет тип uuid (см. миграцию 0015).
- **Биллинг Apify через `costOverrideUsd`** в `recordCall` — добавили опциональный параметр, потому что pricing-таблица знает только token-based стоимость. Apify считается по `~$0.0023/пост × N`.
- **Транскрипция (Whisper) для видео-постов** — не реализована: в Сессии 33 ТЗ упоминает её опционально, и сегодня нет интегрированного механизма скачивания видео и прогона через Whisper. Поле `transcription` в таблице есть и будет наполняться позже (Сессия 41 «голосовые + Whisper»).
- **AI-саммари использует первого доступного дешёвого провайдера** (Anthropic Haiku → OpenAI 4o-mini → Gemini Flash). Сессия 49 даст Системную LLM — там переключусь.
- **`apify_token_present: false` на текущем проде** — ENV `APIFY_TOKEN` не задан Владом (он узнает по красной полосе в Базе конкурентов и плашке в Админке). Кнопка «Добавить блогера» disabled до установки. UI и API готовы; реальное добавление блогера — manual check.
- **Apify Actor `apify/instagram-scraper`** — выбран как самый распространённый. Если у Влада на акаунте Apify будет другой scraper, поменять — одна строка `ACTOR_ID` в `apifyService.js`.
- **Размер поста для AI-саммари обрезаются 2000 символов** (caption + transcription). Длинные подписи сравнительно редки в Reels; экономит токены на дешёвой модели.

---

### Сессия 34 — Мерджинг артефактов и Мульти-LLM клонирование (этап 5, пункт 17) ✅ 2026-05-12
...
**Отклонения:**
- **`source_artifact_ids` хранится в шапке merged-артефакта**, а не в отдельной БД-колонке: артефакты в Storage идентифицируются путями, не uuid'ами. Merged-файл начинается секцией «## Источники» со списком исходных путей — этого достаточно для трассируемости. БД-колонка появится, если когда-то понадобится строгая JOIN-навигация.
- **mergeService использует дешёвую модель** (Haiku/4o-mini/Flash) и `maxTokens=8192`. Дорогую модель для объединения берут вручную — переключается одной строкой в `pickProvider`, когда подключим Системную LLM (Сессия 49).
- **«Сравнить с другой моделью» использует существующий `ModelSelector`** (без doc'а disabled prop), модалка показывается поверх TaskViewerModal. После клонирования — `window.location.href` на страницу сравнения, не Next-router, чтобы избежать сложной координации с открытыми модалками.
- **Страница сравнения N-колоночная** (не строго две). Если у группы есть оригинал + 2+ клонов — все три (и более) рендерятся бок о бок через `gridTemplateColumns: repeat(N, 1fr)`. На широких экранах удобно, на узких остаётся горизонтальный скролл.
- **`comparison_group_id` — hex16 строка** (из `randomBytes(8).toString("hex")`), не uuid. На уровне БД это TEXT, не uuid; для коротких ссылок удобнее короткий id.
- **Кнопка «Сравнение →»** появляется рядом с «Сравнить с другой моделью», если у задачи уже есть `comparison_group_id` — удобно вернуться на страницу группы из любого клона.
- **`/api/team/tasks/compare/:groupId` объявлен ДО `/:taskId`** — иначе Express матчит «compare» как taskId, на нашей `TASK_ID_REGEX=/^tsk_/` это даёт 400.
- **Multi-LLM сравнение клонирует даже clarification/self-review настройки** — задумка: «два варианта одной задачи, разные модели»: одинаковое всё, кроме модели. Если Влад хочет сравнить с/без чекбокса самопроверки — это уже другой сценарий, делается двумя отдельными постановками.

---

### Сессия 35 — Шаблоны задач разведчика, триггеры, Role-черновик (этап 5, пункт 17) ✅ 2026-05-12
...
**Отклонения:**
- **Шаблон `Глубокий ресёрч через NotebookLM.md` НЕ создаётся в этой сессии** — он зарезервирован для Сессии 37 (Предпродакшн, исследователь). ТЗ Сессии 35 описывает только три задачи разведчика (analyze_competitor, search_trends, free_research).
- **Хендлеры `handleAnalyze*/Search*/FreeResearch*` — тонкие обёртки над общим `handleScoutGeneric`**. Все три задачи делают один LLM-вызов + сохранение артефакта в `research/scout/`. Уникальная логика появится в Сессии 37, если понадобится (например, многошаговый разбор конкурента по сегментам).
- **`callForTask` автоматически подключает Web Search**, если у агента в Hands есть `web-search`. Никаких явных опций в шаблонах не нужно — методичка инструмента уже идёт в Awareness промпта.
- **Триггер `new_competitor_entry` расширен на посты**: после Сессии 33 `team_competitor_posts` наполняется AI-саммари; теперь триггер сработает, если в базе появились новые посты, даже без появления нового блогера. Защита от падения на старых окружениях через `relation .* does not exist` regex.
- **Триггер `new_reference_entry`** проверяет `public.videos.created_at > cutoff`. Cooldown 7 дней, как у других событийных. Если таблицы `videos` нет — тихий skip.
- **`role-scout.md` — справочный файл в scripts/templates/**, НЕ загружается в Storage автоматически. Влад создаёт агента через мастер (Сессия 10) и копипастит черновик в Role.
- **Web Search в шаблоне analyze-competitor — рекомендация, не обязательное условие**. Если у разведчика в Hands не привязан Web Search, агент справится без него по brief'у `competitor_data`. Это удобнее для тестов без Tavily/Perplexity-ключа.
- **`competitor_data` плейсхолдер не подставляется автоматически** — Влад вручную копирует данные из `/blog/databases/competitors/` в бриф задачи. Автоматическая выборка по `competitor_id` — потенциальное улучшение Сессии 37+.
- **Обновлены 3 фронт-словаря**: `TEMPLATE_META` (TaskCreationModal), `TASK_TYPE_LABELS` (taskTypeMeta). Третий каталог `TEMPLATE_NAMES` в backend taskHandlers + дубликат `TASK_TYPE_TO_SLUG` в promptBuilder синхронизированы. Это знакомое дублирование (Сессия 29) — устранится, когда Сессия 49 даст реестр шаблонов в БД.

---

### Сессия 36 — Bugfix загрузки файлов и финальная интеграция (этап 5, пункт 17) ✅ 2026-05-12
...
**Отклонения:**
- **Backend route `/api/team/files/upload` уже был корректным** — multer + memory storage + sanitizePath + dedup-timestamp. ТЗ говорит «починить» как если бы он был сломан, но фактический баг — отсутствие UI кнопки на форме постановки задачи (не сам endpoint).
- **`FileAttachmentField` компонент в TaskRunnerModal** — `accept=".pdf,.docx,.txt,.md,.png,.jpg,.jpeg"` (как просит ТЗ), лимит 50 МБ на бэкенде (не 10 МБ из ТЗ — current `files.js` стоял на 50 ещё до Сессии 36; не уменьшаю, чтобы не ломать существующее поведение).
- **Биллинг Apify** — отдельная строка появится в `SpendingSection.by_provider` АВТОМАТИЧЕСКИ как только в `team_api_calls` появятся записи с `provider='apify'` (запускается через competitorService → recordCall с purpose='apify' и provider='apify'). Существующий UI выводит все провайдеры из агрегатов — никаких хардкодов не нужно. Пока на проде нет APIFY_TOKEN — провайдер не появился, но как только парсинг запустится, строка возникнет сама.
- **Карточка инструмента Apify в Админке** (отдельный блок «Инструменты Системы») уже была реализована в Сессии 33 (`ApifyCard`). Не дублирую.
- **`test:p17` интеграционный тест — 7 проверок**: Web Search tool/manifest, team_competitor_posts, реестр конкурентов, taskContinuationService, 3 scout-шаблона, mergeArtifacts (с реальным LLM-вызовом). На проде проходит 7/7.
- **`new value` для интеграционного теста** — путь файла в имени уникальный (timestamp), но cleanup делается best-effort (errors silently ignored). Тестовый merge-артефакт остаётся в `merges/` — Влад может проверить вручную, что текст склеен корректно. Это полезно как QA-артефакт.

---

### Сессия 37 — Шаблоны задач предпродакшна и черновики Role (этап 5, пункт 18) ✅ 2026-05-12
...
**Отклонения:**
- **Загружено ровно 13 шаблонов** (4+3+3+3). На проде сейчас 21 task type: 5 базовых + 3 scout + 13 preprod.
- **Все 13 hendler'ов — тонкие обёртки над `handleScoutGeneric`** (один LLM-вызов + сохранение артефакта в `research/preprod/<role>/<kind>/...`). Уникальная логика для NotebookLM multistep — Сессия 38; пока `deep_research_notebooklm` идёт как обычный одноразовый вызов (frontmatter `multistep: true` зарезервирован, но handler пока его не читает).
- **Storage-пути для предпродакшн-артефактов: `research/preprod/<role>/<ts>_<kind>_<slug>.md`** — единое дерево для всех 4 ролей: researcher / scriptwriter / factchecker / chief. Удобно для последующих handoff'ов и просмотра в Артефактах.
- **Черновики Role в `scripts/templates/role-{researcher,scriptwriter,factchecker,chief-editor}.md`** — справочные файлы, НЕ загружаются в Storage автоматически. Влад создаст агентов через мастер, скопировав текст в редактор.
- **Шеф-редактор получил блоки «Когда я смотрю на свою зону» и «Когда я выхожу с предложением»** для autonomy_level=1 (по образцу role-scout). У трёх остальных предпродакшн-агентов автономности нет — это сознательное решение ТЗ Сессии 38.
- **Frontmatter `multistep: true` в deep_research_notebooklm** добавлен для Сессии 38, которая реализует NotebookLM-multistep flow через `taskContinuationService`. Сегодня этот ключ ни на что не влияет.
- **На фронт добавлены 13 новых записей в `TEMPLATE_META` (TaskCreationModal)** + `TASK_TYPE_LABELS` (taskTypeMeta). Видны в form-step 2 сразу.

---

### Сессия 38 — Многошаговый ресёрч NotebookLM и end-to-end пайплайн (этап 5, пункт 18) ✅ 2026-05-12
...
**Отклонения:**
- **Реальной интеграции с NotebookLM API нет**. Notebook ID идёт в промпт как метаданные, а каждый шаг выполняется обычным `callForTask` (с подключением Web Search, если у агента есть). Полноценный NotebookLM-воркер с локальной CLI/Apify-связкой — за рамками этой сессии (ТЗ помечает «реальный NotebookLM-воркер — Сессия 38+»).
- **Recovery awaiting_resource через worker-pool**: в `getActiveTaskIds` добавлен этот статус. Когда runner подхватывает такую задачу, handler видит `task.step_state` и возобновляет с `current_step`. Это работает для in-process multistep; для реального external-воркера потребуется отдельный поллер.
- **Многошаговая логика — inline в handler'е**, не через отдельный сервис-воркер. На каждом шаге `persistStepState` пишет append-only снапшот → UI поллит `team_tasks` и показывает прогресс. Это упрощает реализацию, ценой того, что весь multistep блокирует один слот воркер-пула (TEAM_WORKER_CONCURRENCY=1 → одна multistep задача занимает всю очередь). При увеличении CONCURRENCY проблема исчезает.
- **`buildStepUserPrompt` подмешивает 3 последних Q/A** в каждый запрос — для согласованности между шагами. Это даёт небольшой контекст из предыдущих ответов без раздувания токенов всей истории.
- **`buildSynthesisUserPrompt`** на последнем шаге даёт LLM всю аккумулированную фактуру и просит собрать единый артефакт. Используется тот же agent.model — не переключаемся на дорогую модель синтеза (это решает Системная LLM Сессии 49).
- **`test-pipeline-e2e.js` — структурный тест**, не интеграционный с реальной БД. Проверяет: парсер questions_list, цикл `initMultistepTask` + `continueTask`, корректность step_state при resume, структурная цепочка handoff'ов. 5/5 проходят локально без сетевых вызовов.
- **UI deploy задержка** — Vercel может занять 2-5 минут после push. На момент финализации Сессии 38 поля «Notebook ID / Список вопросов» в форме могли ещё не появиться в браузере — TS check и build прошли локально, код в main. См. TEST_REPORT для ручной верификации после деплоя.
...
### Сессия 40 — Ежедневные отчёты и push-уведомления о готовых задачах (этап 6, пункт 20) ✅ 2026-05-12
...
**Отклонения:**
- **Push-уведомления НЕ использует Системную LLM**: ТЗ предлагал «одно предложение через Системную LLM», но это лишний расход на каждой done-задаче. Беру первые 200 символов `task.result`, обрезаю whitespace — этого хватает для preview-блока в Telegram. Если хочется LLM-summary — можно вернуть в Сессии 49 при настройке Системной LLM.
- **`tickDailyReports` зовут каждую минуту**, но он сам проверяет `matchesTime` (HH:MM в timezone). При совпадении пишет `last_report_date` в team_settings и пропускает повторы в ту же минуту. Это надёжнее ETag-логики через cron-period с timezone-aware расписанием.
- **Защита от двойной отправки** через `telegram_last_report_date` (one row per day, формат YYYY-MM-DD в settings.timezone). Cron каждую минуту проверяет → если today_date уже в last_report_date, пропуск.
- **purpose='telegram_report'** для отчёта, **purpose='task_done'** в `source_type` очереди (если попадёт в quiet hours). Биллинг Сессии 49 разнесёт эти строки по категориям.
- **Если задач у агента за сегодня нет — отчёт НЕ отправляется**. Это сознательно: «сегодня ничего не делал» — шум; молчание = «ничего не было».
- **Fallback-отчёт** (если LLM-вызов упал): простая HTML-разметка из списка задач + ссылки. UX похуже, но не теряем уведомление.
- **`FRONTEND_PUBLIC_URL` через ENV** — дефолт `https://potok-omega.vercel.app`. Если меняется домен — одно изменение в Railway → перезапуск.
- **Динамический импорт `pushTaskDoneNotification`** в taskRunner — чтобы избежать циклической зависимости (jobs/dailyReportsJob → telegramService, который в свою очередь не зависит от taskRunner, но всё равно осторожно).
- **`escapeHtml` в HTML-блоках** — на случай странных названий задач со спецсимволами. Telegram parse_mode=HTML.
...
### Сессия 42 — Интеграционное тестирование Telegram и финализация (этап 6, пункт 20) ✅ 2026-05-13
...
**Отклонения:**
- **Большая часть edge-cases уже была покрыта** в Сессиях 39-41: retry-after 429 (max 3 попытки), guard `bot.status !== "active"`, тихий пропуск без `TELEGRAM_SYSTEM_BOT_TOKEN`. В этой сессии оставил их как есть, не переписывая, и записал в test-telegram.js проверочные пути.
- **Paused-агент в daily report**: до Сессии 42 `sendAgentReport` НЕ проверял статус агента — если бот в `team_telegram_bots` активен, отчёт улетал даже paused/archived агенту. Добавил один гард после `getAgent()`. Это маленькое расхождение с поведением до Сессии 42, но соответствует ТЗ: «dailyReportsJob пропускает его».
- **`sendAgentReport`** перенесён в `export` — раньше был internal-only, нужен для теста 6.
- **Объявления состава команды (👋⏸▶️📦)** идут через `announceAgentRosterChange(kind, displayName)` в `telegramService.js` и вызываются из `agentService.createAgent` / `setStatus` через `setImmediate` + динамический импорт — те же приёмы, что в Сессиях 18/41 для избегания цикла модулей.
- **Тест 6 (paused agent)**: dummy-task вставляется в `team_tasks` без поля `user_id` — такого столбца в схеме нет (старый test-telegram-session-40.js ошибочно прокидывал `user_id`, Supabase отбивает с `PGRST204`; в этой сессии корректную форму insert'а заодно зафиксировал).
- **Frontend build на dev-машине** падает на «collect page data» из-за отсутствующих `NEXT_PUBLIC_SUPABASE_URL` в `.env.local` — это pre-existing (зафиксировано в TEST_REPORT начиная с Сессии 14), на Vercel ENV инжектится. Compile + типы + lint прошли до этой стадии.

---

### Сессия 43 — Уникальные ссылки на задачи и полноэкранный режим (этап 6, пункт 22) ✅ 2026-05-13
...
**Отклонения:**
- **Полноэкранная страница = TaskViewerModal на пустом фоне**, а не отдельный «expanded»-layout. TaskViewerModal — 945 строк, дублировать всю эту логику отдельным UI было бы вредно. Модалка живёт `fixed inset-0`, на чистой странице визуально занимает весь экран; `onClose` навигирует `router.push('/blog/team/dashboard')` (а не `router.back()` — для случая прихода по прямой ссылке из Telegram, когда history пустой).
- **Цепочка parent/child со ссылками внутри карточки задачи** — уже была реализована в Сессии 13 внутри TaskViewerModal (заголовок «← из задачи …» с переходом к родителю). Дополнительная навигация на странице не нужна — модалка её уже даёт.
- **Кнопка «Скопировать ссылку» в шапке полноэкранной страницы** не добавлена отдельно: иконка ↗+🔗 уже на карточке в логе; URL виден прямо в адресной строке браузера. При открытой странице Влад копирует URL стандартным способом (Cmd+L → Cmd+C). Если в продакшене окажется нужным — добавим header-кнопку отдельным микро-патчем.
- **TaskCard** переведён с `<button>` на `<div>` + absolute-overlay + 2 icon-кнопки (Maximize2 / Link2). Иконки видимы только на hover (`opacity-0 group-hover:opacity-100`), чтобы не засорять карточку. Аналогичный приём — в StaffWorkspace из Сессии 19.
- **Telegram-ссылки**: daily reports и push-уведомления о done-задачах УЖЕ использовали `/blog/team/tasks/${id}` (Сессия 40, `FRONTEND_URL`). Дополнительно перенаправил Inbox-нотификации `task_awaiting_review` и `handoff_suggestion` с `/blog/team/dashboard` на `/blog/team/tasks/<id>` — теперь клик в Telegram inline-кнопке/ссылке ведёт прямо на задачу.
- **`navigator.clipboard` fallback**: если браузер заблокировал clipboard (HTTP / iframe / отозванный permission) — копирование валится в `window.prompt('Скопируйте ссылку:', url)`. Не теряем сценарий.
- **`NEXT_PUBLIC_SITE_URL`**: при формировании копируемого URL предпочитаем ENV-переменную (когда захотим — добавим в Vercel `NEXT_PUBLIC_SITE_URL=https://potok-omega.vercel.app`); fallback на `window.location.origin`.

---

### Сессия 44 — Batch-режим Anthropic API (этап 6, пункт 22) ✅ 2026-05-13
...
**Отклонения:**
- **Миграция — `0033_team_tasks_batch_mode.sql`** (сквозная нумерация проекта), не `0028` как в локальном ТЗ.
- **Batch-режим поддерживается для всех типов задач, КРОМЕ `edit_text_fragments`**: тот тип требует runtime-fetch parent artifact, плохо подходит под подход «отправил промпт и ушёл». В runTaskInBackground для batch+anthropic+edit_text_fragments → warning + обычное выполнение.
- **`task.prompt` используется как есть** (из createTask), без пересборки на момент submission. Для типов с runtime-обогащением (research_direct/write_text) source/research уже зафиксированы в createTask — batch отправляет ровно тот промпт, который должен был отправиться сразу. Это сознательно: batch не даёт «более свежий» источник, чем обычная очередь.
- **Артефакт пишется унифицированно**: `batches/<task_id>.md` с минимальным header'ом, без per-type форматирования. Per-type handlers (`handleResearchDirect`, `handleWriteText` и т.п.) делают разные шапки артефакта — повторять их логику в `batchPollService` было бы дублированием. UI показывает результат из `task.result`, артефакт в Storage нужен для архива.
- **`source = 'batch'` в биллинге** — пишется как `purpose='batch'` в `team_api_calls` (как у всех прочих purpose-кодов, начиная с Сессии 22). Скидка 50% реализована через новый параметр `costMultiplier: 0.5` в `recordCall`.
- **`batch_default` в frontmatter шаблонов**: НЕ добавлен — ни один существующий шаблон не имеет осознанной потребности по умолчанию идти в batch. Пользователь явно ставит галку, когда хочет.
- **`anthropic_batch_enabled` в team_settings** хранится как обычный jsonb-key-value (как `autonomy_enabled_globally` из Сессии 22).
- **`startBatchCron` всегда стартует**, но молчит при выключенном флаге (внутренний кэш 30 сек). Это надёжнее «процесс перезапускать при смене настройки».
- **`batch_id` не очищается в финале** — оставлен для трассируемости (Влад может увидеть в БД, какой именно batch произвёл этот результат).
- **Self-review для batch-задач не запускается**: после batch-result задача сразу идёт в `done`. Если бы делали self-review через batch — это второй raw batch, ещё 24ч ожидания; через прямой call — теряется идея batch'а. На MVP принимаю: batch и self-review взаимоисключающие, фронтенд может добавить взаимное disable позже.

---

### Сессия 45 — Кастомные базы с нуля: мастер создания (этап 6, пункт 22) ✅ 2026-05-13
...
**Отклонения:**
- **Миграция — `0034_team_custom_db_functions.sql`** (сквозная нумерация), не `0029` из локального ТЗ. SQL-функция `public.create_custom_table(p_table_name TEXT, p_columns JSONB)` с `SECURITY DEFINER` и `SET search_path = public`. Дополнительная защита: имя таблицы обязано матчить `^team_custom_[a-z0-9_]+$`, имена колонок — `^[a-z][a-z0-9_]*$`, зарезервированные `id`/`created_at` отбиваются. PL/pgSQL валидирует тип JSON-аргумента (`jsonb_typeof(p_columns) = 'array'`).
- **Имя таблицы генерируется бэкендом**: `team_custom_<slug>_<timestamp-base36>`. Slug — транслитерация имени базы (latin/digits/underscores). Это значит, что Влад НЕ выбирает SQL-имя и не может его сломать. Имена колонок Влад вводит сам (через UI мастера), валидация регэксп в сервисе + RAISE в SQL-функции.
- **`schema_definition`** — единственный источник правды о `label` и `options` (real Postgres-таблица их не знает: реальная схема знает только колонки `text/numeric/text[]/...`). В `validateRecordPayload` мы используем `key`+`type`+`options` чтобы запретить запись неизвестных полей и не дать булеву прийти строкой.
- **CRUD UI**: вместо переписывания серверной таблицы под клиент я добавил островок `CustomDbRecordEditor` НАД серверной таблицей. Он показывает чипы записей с кнопками ✏️ / 🗑 + крупную кнопку «+ Добавить запись». При успехе делается `router.refresh()`, серверная таблица перерисовывается с новыми данными. Это компромисс: меньше дублирования кода рендеринга таблицы, ценой того, что edit/delete-кнопки живут в отдельной полоске под основной таблицей, а не на самой строке. Если станет неудобно — рефакторим в Сессии 47 как часть финализации.
- **Поле дат**: тип `date` сохраняется как ISO `YYYY-MM-DD`. UI input `<input type="date">`. PG-тип `DATE` корректно принимает эту строку.
- **`multi_select` без options** в schema → UI показывает «В схеме базы не заданы варианты», но Postgres-тип `TEXT[]` остаётся валидным (просто всегда пустой массив).
- **Мастер пропускает быстрое создание с одной кнопки**: Влад обязан пройти 3 шага. Кнопка «Далее» disabled, пока не заполнены обязательные поля (имя на шаге 1, имена колонок на шаге 2).
- **Slug автогенерация в UI**: НЕ показываем превью slug — Влад его не выбирает. Если slug пустой (имя из символов вне translit-map) → ошибка после нажатия «Создать базу». Полноценный live-превью slug отложен, если станет частым кейсом.
- **API эндпоинты**: POST `/api/team/databases`, POST/PATCH/DELETE `/api/team/databases/:id/records[/:recordId]`. Все за `requireAuth` (через `router.use(requireAuth)`). Сервис ругается на не-custom базы при попытке CRUD — `Referensy`/`Competitors` остаются read-only.
- **Sidebar**: по факту уже динамический с Сессии 5 — listDatabases выдаёт кастомные записи, sidebar их подбирает автоматически. Никаких правок не понадобилось.

---

### Сессия 46 — Промоут артефакта в базу и дизайн-токены (этап 6, пункт 22) ✅ 2026-05-13
...
**Отклонения:**
- **Промоут артефакта**: реализован через `POST /api/team/artifacts/promote-to-base` (новый эндпоинт, body: `{ artifact_path }`). LLM выбирается из дешёвых через тот же helper `pickProvider()`, что в `mergeService.js` (Сессия 34) — Системная LLM Сессии 49 ещё не существует. Расход пишется через `recordCall({ purpose: 'promote_artifact' })`.
- **JSON-парсинг ответа LLM**: робастный helper `extractJsonObject` — снимает тройные обратные кавычки и берёт substring между первым `{` и последним `}`. Если JSON всё равно не парсится — отдаём пустой suggestion с дефолтным именем (имя файла без расширения).
- **`CreateDatabaseButton` теперь поддерживает controlled-режим** (props `mode: 'controlled' | 'uncontrolled'`). В uncontrolled — рисует свою кнопку (как в Сессии 45). В controlled — снаружи передают `open`, `onClose`, `initial` (с заполненными name/description/columns). PromoteArtifactButton использует второй вариант: открывает мастер с suggestion'ом.
- **PromoteArtifactButton — это row-level кнопка `Database` (lucide)** в `ArtifactBrowser.tsx`, показывается рядом с уже существующей кнопкой удаления для каждого файла (не папки). Spinner на время LLM-вызова, ошибка показывается баннером ниже списка. При успехе — открывается controlled-режим CreateDatabaseButton с готовыми колонками.
- **Дизайн-токены Хокусая**: файл `frontend/src/styles/hokusai-tokens.css` создан с 16 CSS-переменными (palette + статусы), импортирован в `globals.css` через `@import`. **Полная миграция компонентов команды на эти токены НЕ сделана**. Причина: текущая Tailwind-палитра (canvas/surface/ink/line/accent — `#FF5B1F`) живёт в `tailwind.config.ts` как hex-константы. Компоненты используют семантические Tailwind-классы (`bg-canvas`, `text-ink-muted`), а не hardcoded hex — там менять нечего. Полная смена палитры требует переписывания `tailwind.config.ts` под `colors: { canvas: 'var(--bg-canvas)', ... }` И добавления RGB-триплетов для opacity-modifier — это самостоятельная задача дизайна (могут «поплыть» все hover-варианты, accent-soft и т.п.). Токены в файле служат документацией будущей палитры; явная миграция отложена до самостоятельной UI-сессии, когда Влад скажет «делаем редизайн». Сейчас раздел остаётся в существующей тёплой гамме, без визуальной регрессии.
- **`status-*`** CSS-переменные предусмотрены, но пока не подключены к статусным бейджам — те живут в `statusBadge(task.status)` в `taskTypeMeta.ts` и используют Tailwind utility-классы вроде `bg-emerald-100 text-emerald-800`. Перевод на var(--status-done) аналогичен — отложен на ту же UI-сессию.

---

### Сессия 47 — Интеграционное тестирование пункта 22 и финализация этапа 6 (этап 6, пункт 22) ✅ 2026-05-13
...
**Отклонения:**
- **Тест 1 (уникальная ссылка)**: вместо HTTP-вызова `GET /api/team/tasks/:id` тест работает напрямую через Supabase JS client — это интеграционный тест on the same process (бэкенд при запуске script'а не поднимается). Эквивалентность гарантирована тем, что серверный handler (`routes/team/tasks.js`) тоже зовёт `getTaskById`. Полноценный HTTP-E2E с проверкой signing JWT — отдельная задача (можно прогнать вручную через curl).
- **Тест 2 (batch submission)**: не отправляет реальный batch в Anthropic (требует ключ + сеть + 5-15 мин ожидания). Эмулирует поведение `createTask({ batchMode: true })` через прямой INSERT с `batch_mode=true`, `batch_id=null`. Проверка статуса `awaiting_resource` после `submitTaskAsBatch` — отдельный E2E с реальной Anthropic, описан как «требует ручной проверки» в Сессии 44.
- **Тест 3 (кастомная БД)**: при первой попытке упал с `Could not find the table '...' in the schema cache` — PostgREST не успел перезагрузить cache после CREATE TABLE. **Починил** двумя слоями:
  1. **Миграция 0035** добавляет `NOTIFY pgrst, 'reload schema'` в SQL-функцию `create_custom_table` — теперь PostgREST мгновенно узнаёт о новой таблице.
  2. **`waitForTableReady` в `customDatabaseService.createDatabase`** — active poll через `select limit 0` до 5 секунд с шагом 400мс. Без этого первый `addRecord` после `createDatabase` упал бы у Влада в UI с тем же сообщением. После фикса первый INSERT отрабатывает за ~0.5-1с.
- **Тест 4 (Telegram-ссылки)**: source-check вместо runtime-теста. Регекспом ищем `link: \`\/blog\/team\/tasks\/\${taskId}\`` в `taskRunner.js` — это даёт гарантию, что Сессия 43 не сломалась в Сессии 44+. Реальный поток «создать задачу → done → Telegram link → клик» требует Telegram-бота и попадает в ручную проверку.
- **Тест 5 (Хокусай-токены)**: source-check `frontend/src/styles/hokusai-tokens.css` на наличие всех 16 переменных из ТЗ.
- **Pure integration test, не на полноценном HTTP-stack**: запускается через `node`, дёргает сервисы напрямую (без поднятия express). Это намеренно: тесты ловят регрессии в логике, а HTTP-уровень покрывается smoke'ом через Playwright + ручную проверку UI.
- **5 тестов вместо описанной в ТЗ архитектуры "5 тестов" — совпадает** с ТЗ.
- **Cleanup**: `cleanupTestData()` в finally удаляет test-задачи и test-записи реестра. Реальные таблицы созданных тест-баз НЕ удаляются (Supabase JS API не умеет DROP TABLE) — они накапливаются как `team_custom_p22test_*_<timestamp>`. При следующем прогоне создадутся новые с другим timestamp. Чистить вручную через Dashboard, если станет много.
...
### Сессия 50 — Мониторинг NotebookLM и финализация Админки (этап 7, пункт 1) ✅ 2026-05-13
...
**Отклонения:**
- **Миграция — `0038_team_notebooklm.sql`** (сквозная нумерация), не `0032` из локального ТЗ. Помимо `team_notebooklm_heartbeat` миграция СОЗДАЁТ ТАКЖЕ `team_notebooklm_queue` — ТЗ Сессии 20 упоминала её как «🔁 пункт 17», но de facto она не была создана в прошлых сессиях. Без очереди эндпоинт `queueTestTask()` не работает.
- **`team_notebooklm_queue` shape**: id UUID PRIMARY KEY DEFAULT gen_random_uuid(), type TEXT, payload JSONB, status TEXT CHECK ('queued','running','done','error'), result JSONB, error TEXT, created_at/started_at/completed_at. Минимально достаточно для тестового сценария и реального воркера в этапе 5/17. Партициальный индекс по status WHERE status IN ('queued', 'running') — для быстрой выборки задач воркером.
- **Пороги статусов**: 🟢 <1 мин, 🟡 1–5 мин, 🔴 >5 мин. Совпадает с ТЗ.
- **UI auto-refresh 30 сек** через `setInterval`. На размонтировании компонента очищается. Polling теста — каждые 3 сек, максимум 30 сек (10 попыток), затем «Таймаут».
- **«Финализация Админки»** (по ТЗ — порядок блоков Безопасность → Ключи → Системная LLM → Расходы → NotebookLM → Telegram → Проактивность): большая часть порядка уже совпадает с текущим layout AdminWorkspace.tsx (ProvidersSection → SystemLLMSection → SpendingSection → NotebookLMSection → ... → TelegramSection → AutonomySection). Полный реоrder с visual-консистентностью отложен — текущий порядок логичен и не требует ломки.
- **Hokusai-tokens.css упомянут в критериях**, но фактическая миграция компонентов на CSS-переменные отложена (см. отклонения Сессии 46). Текущий блок NotebookLMSection использует Tailwind-классы как все остальные секции.

---

### Сессия 51 — Интеграционное тестирование пункта 1 и финализация этапа 7 (этап 7, пункт 1) ✅ 2026-05-13
...
**Отклонения:**
- **Тест 1 (custom провайдер)** при первом запуске упал на `team_api_keys_provider_check` — CHECK constraint из миграции 0012 (этап 1) ограничивал `provider IN ('anthropic', 'openai', 'google')`. Сессия 48 расширила JS-сервис под произвольных провайдеров, но забыла снять CHECK на уровне БД. **Починил** миграцией `0039_team_api_keys_drop_check.sql` — `ALTER TABLE ... DROP CONSTRAINT team_api_keys_provider_check`. Это критичный фикс UX: без него Влад **не мог бы** добавить DeepSeek/Groq через UI Админки Сессии 48 на проде — каждая попытка возвращала бы `team_api_keys_provider_check` от PostgREST. Валидация slug провайдера остаётся в `keysService.ensureProvider` (regex `^[a-z][a-z0-9_-]{0,40}$`).
- **Тест 2 (presets)**: проверяет наличие 8 обязательных id (anthropic, openai, google, deepseek, groq, perplexity, openrouter, ollama_cloud) и поле `help_url` у всех. PROVIDER_PRESETS — статический объект из `config/providerPresets.js`, не требует БД.
- **Тест 3 (System LLM)**: rollback в `finally` восстанавливает оригинальную конфигурацию. Тест меняет на `anthropic/claude-haiku-4-5/$42`, проверяет, что reread возвращает то же, потом восстанавливает.
- **Тест 4 (биллинг)**: не валидирует значения (на пустой БД они нулевые), только что код не падает и возвращает все 5 группировок (total_usd + 4 buckets). Полноценная сверка значений — требует фикстур, отложено.
- **Тест 5 (NotebookLM)**: вставляет тестовый heartbeat → проверяет, что `getStatus()` вернёт `green` с правильной `version='p1-test'`. Cleanup в finally удаляет вставленную запись.
- **Cleanup в `finally`**: удаляет test провайдеров через `deleteApiKey`, heartbeat-записи через DELETE, восстанавливает System LLM. На случай неуспеха одного из шагов — try/catch, продолжаем дальше.
- **HTTP-уровень не покрыт**: тесты работают через сервисы напрямую, как в Сессии 47 (`test-p22.js`). Express HTTP-маршруты — обёртки над теми же сервисами, тестируются через UI и curl.
